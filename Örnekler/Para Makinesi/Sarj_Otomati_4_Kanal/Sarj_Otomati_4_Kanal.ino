...
volatile bool paraGeldimi = false; // ekranda gelen parayı ifade etmek için kullanılacak! bu değere göre ekranda para geldiğinde değişiklik olacak!
...
void loop() {
  ...
  // Ekran modu sadece şarj varken değişsin. Röleler kapalıyken sadece ana menü gösterilsin
  if(sarjVarmi) {
    if(millis() - sonEkranDegisimi > ekranDonguSuresi) {
      ekranModu = ekranModu == 0 ? 1 : 0;
      modGosterildimi = false;
...
void roleCalistir(byte roleNo) {
  ...
  if(roleNo == 1) {
    if(role_1_Acikmi) {
      ...
      role_1_bitmeSuresi += roleAcikKalmaSuresi;
    } else {
  ...
  if(roleNo == 2) {
    if(role_2_Acikmi) {
      // röle zaten açıksa süre ekle!
      role_2_bitmeSuresi += roleAcikKalmaSuresi;
    } else {
      // röle açık değilse bitme süresini ayarla. ve çalıştır!
  ...
  if(roleNo == 3) {
    if(role_3_Acikmi) {
      // röle zaten açıksa süre ekle!
      role_3_bitmeSuresi += roleAcikKalmaSuresi;
    } else {
      // röle açık değilse bitme süresini ayarla. ve çalıştır!
  ...
  if(roleNo == 4) {
    if(role_4_Acikmi) {
      // röle zaten açıksa süre ekle!
      role_4_bitmeSuresi += roleAcikKalmaSuresi;
    } else {
      // röle açık değilse bitme süresini ayarla. ve çalıştır!
...
void EkranGoster() {
  if(ekranModu == 0 && modGosterildimi == false) {
    lcd.clear();
    lcd.home();
